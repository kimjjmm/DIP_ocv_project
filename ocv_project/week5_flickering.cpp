...
Mat doFlic(Mat srcImg) {
	...
	Point minLoc, maxLoc;
	minMaxLoc(magImg, &minVal, &maxVal, &minLoc, &maxLoc);//최대값 최소값 얻어서 정규화
	normalize(magImg, magImg, 0, 1, NORM_MINMAX);

	Mat maskImg = Mat::ones(magImg.size(), CV_32F); //ones로 모든 원소가 1인 행렬 반환해놓기
	line(maskImg, Point(maskImg.cols /2, 0), Point(maskImg.cols / 2, maskImg.rows), Scalar::all(0), 25, 8, 0); //maskimg에 영역 생성
	...
	//역변환 전 flickering 현상이 지워진 magnitude 이미지 출력
...
int main() {
	Mat src_img = imread("C:\\DIP_Pic\\5\\img3.JPG", 0); // 이미지 읽기
	...
	Mat flicimg = doFlic(src_img);//flickering 현상 제거

	imshow("origin", src_img); //원본영상 출력
	imshow("originmag", nor); //원본영상의 magnitude영상 출력
